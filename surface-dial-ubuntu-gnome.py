...
from evdev.ecodes import EV_KEY, EV_REL, REL_WHEEL, BTN_MOUSE
            ...
            if (event.code == 7 and event.type == EV_REL and event.value < 0):
                ...
                if event.value != -1:
                    print("    UNEXPECTED COUNTER CLOCKWISE VALUE",
                          event.value)
                write(ui, event, EV_REL, REL_WHEEL, 1)

            elif (event.code == 7 and event.type == EV_REL
                  and event.value > 0):
                print("TURN CLOCKWISE")
                if event.value != 1:
                    print("    UNEXPECTED CLOCKWISE VALUE", event.value)
                write(ui, event, EV_REL, REL_WHEEL, -1)
            elif (event.code == 256 and event.type == EV_KEY
                  and event.value == 1):
                print("CLICK DOWN")
                write(ui, event, EV_KEY, BTN_MOUSE, 1)
            elif (event.code == 256 and event.type == EV_KEY
                  and event.value == 0):
                print("CLICK UP")
